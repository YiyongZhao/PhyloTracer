...
Description:
   ...
   hybrid-species gene, the algorithm walks upward to the nearest S (speciation)
   node and inspects the sister clade. If the sister clade contains exactly one
   parental species tag and no other hybrid genes, the gene is assigned the
   corresponding subgenome label (A, B, ...). Labels are derived from the sorted
   order of ``--parental_sps``. Optional ``--input_fasta`` triggers FASTA
   partitioning of the hybrid proteome into per-subgenome files.
...
Optional Parameters:
    - ``--gd_support``      Minimum support of GD nodes used for pair extraction
                            (range: 0-100, default is 50).
    - ``--pair_support``    Minimum support of ortholog/speciation pair nodes
                            (range: 0-100, default is 50).
    - ``--parental_sps``    Parental species names for subgenome assignment;
                            space-separated, quoted string (e.g., ``"ARD ARI"``).
                            Requires ``--hyb_sps``.
    - ``--hyb_sps``         Hybrid species name for subgenome assignment.
                            Requires ``--parental_sps``.
    - ``--input_fasta``     Input FASTA file (.fa/.fasta). If provided, triggers
                            per-subgenome FASTA partitioning using assignments
    ...
    - ``--size``            Point size in dotplot rendering (positive float, default is 0.0005).
    - ``--output_dir``      Output directory (default: current working directory).
