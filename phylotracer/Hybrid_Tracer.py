...
def get_gd_count_dic_and_gd_type_dic(
    ...
    for tre_id, tre_path in tre_dic.items():
        ...
        gds = find_dup_node(t1, rename_sptree)
...
def run_hyde_from_matrix_integrated(
    ...
    with open(tmp_imap_path, "w") as imap_file:
        for species in clean_matrix.index:
            sp_0 = voucher2taxa_dic.get(species, species)  # guard against missing voucher
...
def matrix_to_phy(
    ...
    for col in clean_matrix.columns:
        for species, gene_id in clean_matrix[col].items():
            sp = voucher2taxa_dic.get(species, species)  # guard against missing voucher
...
def process_one_gd(gd_clade: object, outgroup_gene: str):
    ...
    for i, triple in enumerate(result):
        ...
        for species in all_species:
            ...
            if genes_in_species:
                column_data.append(genes_in_species[0])  # pick first gene for multi-copy species (was ";".join which fails seq_dic lookup)
            else:
                column_data.append("-")
