...
from phylotracer.GD_Loss_Visualizer import load_visualizer_species_tree, visualizer_sptree
...
Ortho_Retriever_parser.add_argument('--add_outgroup', action='store_true', default=False, help='Attach strict sister-clade outgroup genes to output trees. Outgroup species must not overlap with ingroup species and must be single-copy within the candidate sister clade.')
...
Hybrid_Tracer_parser.add_argument('--outgroup_species', metavar='SPECIES', default=None, help='Explicit outgroup species to use for outgroup-gene selection. When used with --mrca_node, Hybrid_Tracer will only use genes from this species.')
...
def handle_phylo_tree_collapse_expand(cli_args):
    if cli_args.input_GF_list and cli_args.support_value is not None:
        ...
        collapse_expand_main(tre_dic, cli_args.support_value, revert=cli_args.revert, output_dir=output_dir)
...
def handle_tree_visualizer(cli_args):
    ...
    if cli_args.input_GF_list and cli_args.input_imap:
        ...
        view_main(
            ...
            dup_species_num=cli_args.dup_species_num,
            dup_species_proportion=cli_args.dup_species_proportion,
            deepvar=cli_args.deepvar,
        )
        report_execution_time(start_time)
    else:
...
def handle_gd_loss_tracker(cli_args):
    if cli_args.input_GF_list and cli_args.input_sps_tree and cli_args.input_imap:
        ...
        _, _, filtered_records = get_path_str_num_dic(
            tre_dic,
            sptree,
            gene2new_named_gene_dic,
            new_named_gene2gene_dic,
        ...
        write_gd_loss_csv(filtered_records, 'gd_loss.csv', 'numed_sptree.nwk', node_count_mode=cli_args.node_count_mode)
        write_gd_loss_node_summary_tsv(
            filtered_records,
...
def handle_gd_loss_visualizer(cli_args):
    if cli_args.input_sps_tree and cli_args.gd_loss_result:
        start_time = time.time()
        sptree = load_visualizer_species_tree(cli_args.gd_loss_result, cli_args.input_sps_tree)
        visualizer_sptree(
...
def handle_ortho_retriever(cli_args):
    if cli_args.input_GF_list and cli_args.input_imap and cli_args.input_gene_length:
        ...
        split_main(
            ...
            add_outgroup=cli_args.add_outgroup,
            gene_to_species=read_and_return_dict(cli_args.input_imap),
        )
        report_execution_time(start_time)
    else:
...
def handle_hybrid_tracer(cli_args):
    if cli_args.input_GF_list and cli_args.input_Seq_GF_list and cli_args.input_sps_tree and cli_args.input_imap:
        ...
        requested_outgroup_species = cli_args.outgroup_species.strip() if cli_args.outgroup_species else None
        if cli_args.mrca_node:
            for pair_str in cli_args.mrca_node:
                if ',' not in pair_str:
                    logger.warning("Invalid mrca_node format '%s'. Skipping.", pair_str)
        ...
        if target_node_name:
            ...
            if requested_outgroup_species:
                logger.info(
                    "Explicit outgroup species requested for Hybrid_Tracer: %s",
            ...
            if requested_outgroup_species:
                logger.info(
                    "Explicit outgroup species requested without --mrca_node (ignored): %s",
                    requested_outgroup_species,
                )
        ...
        hyde_main(
            ...
            requested_outgroup_species=requested_outgroup_species,
        )
        report_execution_time(start_time)
    else:
...
def handle_hybrid_visualizer(cli_args):
    if cli_args.hyde_out and cli_args.input_sps_tree:
        ...
        if cli_args.node:
            hyde_visual_node_main(cli_args.hyde_out, sptree, output_dir=output_dir)
        else:
            hyde_visual_leaf_main(cli_args.hyde_out, sptree, output_dir=output_dir)
        report_execution_time(start_time)
    else:
...
def _normalize_input_paths(args):
    ...
    if hasattr(args, "input_GF_list") and args.input_GF_list:
        ...
        temp_files_to_cleanup.append(args.input_GF_list)
    if hasattr(args, "input_Seq_GF_list") and args.input_Seq_GF_list:
        ...
        temp_files_to_cleanup.append(args.input_Seq_GF_list)
    ...
    try:
        ...
        for tmp_file in temp_files_to_cleanup:
            try:
                ...
                logger.warning("Failed to clean temporary mapping file %s: %s", tmp_file, exc)
