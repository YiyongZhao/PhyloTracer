...
def tips_mark(
    ...
    for node in Phylo_t1.traverse():
        if node.is_leaf():
            ...
            column += 1
    if df is not None:
        add_heat_map_to_node(Phylo_t1, df, new_named_gene2gene_dic, column if "column" in dir() else 1)  # guard against unbound column
            ...
            for k, v in sp_node_dic.items():
                if i.name == k:
                    ...
                    for index, value in enumerate(sorted(v)):
                        ...
                        column = int(index if n == 1 or index < n / 2 else index - n / 2)  # ete3 expects int column
